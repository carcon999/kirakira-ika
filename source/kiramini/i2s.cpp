...
#define SLC_BUF_CNT   (2) //Number of buffers in the I2S circular buffer
...
bool ICACHE_FLASH_ATTR i2s_is_full(){
  return (i2s_curr_slc_buf_pos==i2s_buff_length || i2s_curr_slc_buf==NULL) && (i2s_slc_queue_len == 0);
}
...
void ICACHE_FLASH_ATTR i2s_slc_isr(void) {
  ...
  if (slc_intr_status & SLCIRXEOF) {
    ...
    memset((void *)finished_item->buf_ptr, 0x00, i2s_buff_length);//zero the buffer so it is mute in case of underflow
...
void ICACHE_FLASH_ATTR i2s_slc_begin(){
  ...
  for (x=0; x<SLC_BUF_CNT; x++) {
    i2s_slc_buf_pntr[x] = (uint32_t*)malloc(i2s_buff_length);
    for (y=0; y < (i2s_buff_length / sizeof(uint32_t)); y++) i2s_slc_buf_pntr[x][y] = 0;

    i2s_slc_items[x].unused = 0;
    i2s_slc_items[x].owner = 1;
    i2s_slc_items[x].eof = 1;
    i2s_slc_items[x].sub_sof = 0;
    i2s_slc_items[x].datalen = i2s_buff_length;
    i2s_slc_items[x].blocksize = i2s_buff_length;
...
bool ICACHE_FLASH_ATTR i2s_write(uint32_t* pbuff){
  if (i2s_curr_slc_buf_pos==i2s_buff_length || i2s_curr_slc_buf==NULL) {
  ...
  memcpy(i2s_curr_slc_buf, pbuff, i2s_buff_length);
  i2s_curr_slc_buf_pos = i2s_buff_length;
  return true;
}
...
void ICACHE_FLASH_ATTR i2s_begin(uint32_t sz){

  i2s_buff_length = sz;    // len * RGB * bitflag
  _i2s_sample_rate = 0;
  i2s_slc_begin();

// not use LRCK and SCLK
//  pinMode(2, FUNCTION_1); //I2SO_WS (LRCK)
  pinMode(3, FUNCTION_1); //I2SO_DATA (SDIN)
//  pinMode(15, FUNCTION_1); //I2SO_BCK (SCLK)    // pin6
